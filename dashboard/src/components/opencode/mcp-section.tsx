...
export function McpSection({
  ...
  return (
    <div className="space-y-2">
      <label htmlFor="mcp-name-input" className="text-xs font-medium text-[#777169] uppercase tracking-wider">
        MCP Servers
      </label>
      <div className="space-y-2">
        <div className="flex gap-2">
          ...
          <select
            ...
            className="bg-[#f5f5f5] border border-[#e5e5e5] rounded-lg px-3 py-2 text-sm text-black focus:border-[rgba(0,0,0,0.15)] focus:bg-white focus:outline-none transition-colors"
          >
        ...
        {mcpType === "remote" ? (
          <div className="space-y-2">
            <input
              ...
              className="w-full bg-[#f5f5f5] border border-[#e5e5e5] rounded-lg px-3 py-2 text-sm text-black font-mono placeholder:text-[#aaa] focus:border-black/20 focus:bg-white focus:outline-none transition-colors"
            />
          </div>
        ) : (
          <div className="space-y-2">
        ...
        <div className="space-y-2 pt-2 border-t border-[#e5e5e5]">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[#777169] uppercase tracking-wider">
              Environment Variables
            </label>
            <button
              type="button"
              onClick={onAddEnvRow}
              className="text-xs text-[#4e4e4e] hover:text-black transition-colors flex items-center gap-1"
            >
              ...
              {envRows.map((row, index) => (
                <div key={row.id} className="flex gap-2">
                  <input
                    ...
                    placeholder="KEY"
                    className="w-1/3 bg-[#f5f5f5] border border-[#e5e5e5] rounded-lg px-3 py-1.5 text-sm text-black font-mono placeholder:text-[#aaa] focus:border-black/20 focus:bg-white focus:outline-none transition-colors"
                  />
                  <input
                    type="text"
                    value={row.value}
                    ...
                    placeholder="Value"
                    className="flex-1 bg-[#f5f5f5] border border-[#e5e5e5] rounded-lg px-3 py-1.5 text-sm text-black font-mono placeholder:text-[#aaa] focus:border-black/20 focus:bg-white focus:outline-none transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => onRemoveEnvRow(index)}
                    className="p-1.5 text-[#999] hover:text-red-500 hover:bg-[#f5f5f5] rounded-lg transition-colors"
                  >
          ...
          >
            <div className="flex-1 space-y-2 overflow-hidden mr-4">
              <div className="flex items-center gap-2 text-sm font-mono">
                <span className={isEnabled ? "text-black" : "text-[#777169]"}>{mcp.name}</span>
                <span className="text-[#aaa]">&rarr;</span>
                {mcp.type === "remote" ? (
                  <>
                    <span className={isEnabled ? "text-[#4e4e4e] text-xs" : "text-[#999] text-xs"}>Remote</span>
                    <span className="text-[#777169] text-xs truncate" title={mcp.url}>{mcp.url}</span>
                  </>
                ) : (
                  <>
                    <span className={isEnabled ? "text-emerald-600 text-xs" : "text-[#999] text-xs"}>Local</span>
                    <div className="flex flex-wrap gap-1">
                      {mcp.command.map((cmd, i) => (
                        <span key={i} className="px-1.5 py-0.5 bg-[#f5f5f5] rounded text-[#4e4e4e] text-[10px] whitespace-nowrap">
                          {cmd}
                        </span>
                      ))}
                    </div>
              ...
              {envCount > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-[#f0f0f0] text-[#777169] uppercase">
                    {envCount} Env Var{envCount !== 1 ? "s" : ""}
                  </span>
                  <div className="flex gap-1 overflow-x-auto pb-1">
                    {Object.entries(mcp.environment!).map(([k, v]) => (
                      <span key={k} className="text-[10px] font-mono text-[#999] whitespace-nowrap" title={`${k}=${v}`}>
                        <span className="text-[#777169]">{k}</span>={v}
                      </span>
                    ))}
                  </div>
                </div>
