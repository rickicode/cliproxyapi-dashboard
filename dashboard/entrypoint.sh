  ...
  try {
    ...
    BEGIN
      IF EXISTS (
        ...
        WHERE table_name = 'sync_tokens' AND column_name = 'revokedAt'
      ) THEN
        EXECUTE 'DELETE FROM "sync_tokens" WHERE "revokedAt" IS NOT NULL';
        EXECUTE 'ALTER TABLE "sync_tokens" DROP COLUMN IF EXISTS "revokedAt"';
      END IF;
    END $$;
